const Box = (props) => {
    return (
        <div className='box'>
            ...
            <button onClick={() => props.changeColor(props.id, "orange")}>Orange</button>
            <button onClick={() => props.changeColor(props.id, "black")}>Black</button>
        </div>
    );
  }
  ...
  let black_count = 0, red_count = 0, yellow_count = 0, blue_count = 0, green_count = 0, orange_count=0;
   boxes.forEach(box => {
    if (box.color == "black") {
      black_count++;
